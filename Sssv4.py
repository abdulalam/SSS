...
while filter:
    ...
    if user_filter_input == "A":
    ...
    elif user_filter_input == "CSV":
        
       
            
        # dictionary of lists
        dict = {'location': empty_locations, 'section': "C", 'area': "left", 'ispremium' : "2", 'level' : "H"}
            
        df = pd.DataFrame(dict)

        for index, row in df.iterrows():
            df.at[index,'level'] = row['location'][-1]

            if int(row['location'][0:3]) < 564:
                
                df.at[index,'section'] = "A"
            else:
                df.at[index,'section'] = "B"
            
            if int(row['location'][-4:-1]) < 46:
                
                df.at[index,'area'] = "Back"
            else:
                
                df.at[index,'area'] = "Front"

            if int(row['location'][-4:-1]) < 46:                
                df.at[index,'ispremium'] = "0"
            
            elif row['location'][-1] == "A" or row['location'][-1] == "F" or row['location'][-1] == "G":
                df.at[index,'ispremium'] = "0"
            else: 
                df.at[index,'ispremium'] = "1"
            
        # saving the dataframe
        df.to_csv('GFG.txt',index = False)

        f = open("demofile3.txt", "w")
        
        f.write("INSERT INTO `empty_locations` (`location`, `section`, `area`, `ispremium`, `id`, `level`) VALUES\n") 
        for index, row in df.iterrows():
            entry = "('" + row['location'] + "', '" + row['section'] + "', '" + row['area'] + "', '" + row['ispremium'] + "', NULL, '" + row['level'] + "'),\n" 
            #entry = "hello"
            f.write(entry)
        f.close()    
        ...
        f.write("INSERT INTO `isle_stats` (`id`, `isle`, `empty`) VALUES\n") 
        ...
        for islenum in range(islenum_start, islenum_end):
            ...
            if islenum not in range(599, 607):
                if islenum not in range(542,564):
                    ...
                    entry = "(NULL,'" + str(islenum) + "', '" + str(len(temp_location_holder)) + "'),\n" 
                    #entry = "hello"
                    f.write(entry)
